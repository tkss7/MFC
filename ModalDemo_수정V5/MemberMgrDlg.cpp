...
CMemberMgrDlg::CMemberMgrDlg(CMemberMgr& memberMgr, CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MEMBERMGR_DIALOG, pParent), m_memberMgr(memberMgr)
	, m_strID(_T(""))
	, m_strName(_T(""))
	, m_nPostCode(0)
	, m_strAddress(_T(""))
...
BOOL CMemberMgrDlg::OnInitDialog()
{
	...
	//}
	//GetDlgItem(IDC_EDIT_ID)->EnableWindow(m_strID.GetLength() == 0);
