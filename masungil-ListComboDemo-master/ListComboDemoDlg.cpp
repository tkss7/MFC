...
3. CListBox.GetText()을 이용해서 위치를 주고 문자열을 얻는다
4. 얻은 문자열을 출력한다 
*/
	//1. UI에 입력된 값을 변수로 읽는다
	if (UpdateData() == FALSE) {
	...
	if (nFindIndex >= 0) {
		...
		//선택상태로 설정함 
		m_listBox.SetCurSel(nFindIndex);
...
void CListComboDemoDlg::OnBnClickedButton4()
{
	//여러개 선택된 위치 얻기 
	//1. 선택된 항목수를 얻는다
	//2. 선택된 항목의 위치를 저장할 정수 배열을 할당 한다
	//3. 선택된 항목의 위치를 얻는다 
...
//	int* pSelArray = new int[nSelCount];
	//vector<int> array(nSelCount);
	////3. 선택된 항목의 위치를 얻는다
	//m_listBox.GetSelItems(nSelCount, &array[0]);
	////[0] : 0
	////[1] : 2
	////[2] : 4
	//
	//for (int i = nSelCount - 1; i >= 0; i--) {
	//	m_listBox.DeleteString(array[i]);
	//}
...
// 
	...
	m_comboBox.DeleteString(nCurSel);
}
...
void CListComboDemoDlg::OnBnClickedButton5()
{
	...
	//3. 찾은 문자열이 위치를 이용하여 삭제한다
	if (nFindIndex >= 0) {
		m_listBox.DeleteString(nFindIndex);
		m_comboBox.DeleteString(nFindIndex);
	}
