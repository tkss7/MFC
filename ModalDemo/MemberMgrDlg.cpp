...
BOOL CMemberMgrDlg::OnInitDialog()
{
	...
	//ASSERT((IDM_ABOUTBOX & 0xFFF0) == IDM_ABOUTBOX); 제거
	//ASSERT(IDM_ABOUTBOX < 0xF000); 제거

	//CMenu* pSysMenu = GetSystemMenu(FALSE); 제거
	//if (pSysMenu != nullptr)
	//{
	//	BOOL bNameValid;
...
void CMemberMgrDlg::OnBnClickedOk()
{
	...
	m_strName = m_strName.Trim(); // 공백을 제거한다.
	m_nPostCode;
	...
	if (m_strID.GetLength() == 0) {
		AfxMessageBox(_T("ID를 입력해 주세요"));
		GetDlgItem(IDC_EDIT_ID)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return;
	}

	if (m_strName.GetLength() == 0) {
		AfxMessageBox(_T("이름을 입력해 주세요"));
		GetDlgItem(IDC_EDIT_NAME)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return;
	}

	if (m_strAddress.GetLength() == 0) {
		AfxMessageBox(_T("주소를 입력해 주세요"));
		GetDlgItem(IDC_EDIT_ADRESS)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return;
	}
	
	if (m_strPhone1.GetLength() == 0) {
		AfxMessageBox(_T("전화번호를 입력해 주세요"));
		GetDlgItem(IDC_EDIT_PHONE1)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return;
	}

	if (m_strPhone2.GetLength() == 0) {
		AfxMessageBox(_T("전화번호를 입력해 주세요"));
		GetDlgItem(IDC_EDIT_PHONE2)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return;
	}

	if (m_strPhone3.GetLength() == 0) {
		AfxMessageBox(_T("전화번호를 입력해 주세요"));
		GetDlgItem(IDC_EDIT_PHONE3)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return;
	}
...
bool CMemberMgrDlg::IsEmpty(const CString& strText, LPCTSTR lpszMsg, UINT id) const {
	if (strText.GetLength() == 0) {
		...
		GetDlgItem(id)->SetFocus(); // 컨트롤 변수를 추가 안하고 사용할 수 있는 방법
		return true;
	}
}
