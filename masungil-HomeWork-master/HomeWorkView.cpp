...
void CHomeWorkView::OnDraw(CDC* pDC)
{
	...
	for (const CDrawLine& line : pDoc->m_lines) {
		pDC->MoveTo(line.m_array[0]);
		CPen pen(PS_SOLID, line.m_nWidth, line.m_penColor);
...
void CHomeWorkView::OnMouseMove(UINT nFlags, CPoint point)
{
	if ((nFlags & MK_LBUTTON) == MK_LBUTTON) {
		...
		CPen pen(PS_SOLID, pDoc->m_line.m_nWidth, pDoc->m_line.m_penColor);
		CPen* pOldPen = dc.SelectObject(&pen);

		dc.MoveTo(pDoc->m_line.m_array[pDoc->m_line.m_array.size()-1]);
		dc.LineTo(point);
