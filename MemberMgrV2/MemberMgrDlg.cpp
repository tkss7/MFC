...
void CMemberMgrDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_MALE, m_wndMale);
	DDX_Control(pDX, IDC_CHECK_FEMALE, m_wndFemale);
}
...
BEGIN_MESSAGE_MAP(CMemberMgrDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_FEMALE, &CMemberMgrDlg::OnBnClickedCheckFemale)
END_MESSAGE_MAP()
...
void CMemberMgrDlg::OnBnClickedCheckMale()
{
	...
	if (m_bMale)
	{
		m_wndFemale.EnableWindow(FALSE);
	}
	else
	{
		m_wndFemale.EnableWindow(TRUE);
	}
	UpdateData(FALSE);
...
void CMemberMgrDlg::OnBnClickedBtnUpdate()
{
	...
	auto& it = find_if(m_array.begin(), m_array.end(),
		[strId](auto& pMember) {return pMember->m_strId == strId; });
	if (it != m_array.cend()) {
...
void CMemberMgrDlg::OnBnClickedBtnDelete()
{
	...
	*/
	...
	const auto& it = find_if(m_array.cbegin(), m_array.cend(), 
		//[strId](const auto& pMember){return pMember->m_strId == strId; });
...
void CMemberMgrDlg::OnBnClickedCheckFemale()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData();
	if (m_bFemale)
	{
		m_wndMale.EnableWindow(FALSE);
	}
	else
	{
		m_wndMale.EnableWindow(TRUE);
	}
	UpdateData(FALSE);
}
